...
#' 
#' @importFrom BiocParallel bplapply register MulticoreParam SerialParam
#' @importFrom ggplot2 scale_colour_gradient labs
#' @return Either
#' dm (Diffusion map) with relevanceMap added
#' or relevanceMap as list if dm was not supplied.
#' @export
david_compute_gene_relevance <- function(
	coord = NULL,
	matExpression = NULL,
	dm = NULL,
	k = 20,
	...
	nn.index <- find_knn(matExpression, k, distance = dm@distance)$index
	...
	for (dc in seq(1,scaDCDims)) {
		...
		arr3DPartials[,,dc] <- do.call(rbind, bplapply(seq_len(scaNGenes), function(gene) {
	...
	if (is.null(dm)) {
		...
		dm@data_env$relevanceMap <- relevanceMap
		dm
	}
}
...
#' map per gene given.
#' @export
david_plot_gradient_maps <- function(dm, geneIDs) {
	stopifnot(!is.null(dm@data_env$relevanceMap))
	lsGplotsGradientMaps <- list()
	for (strGene in geneIDs) {
		vecExprValue <- dm@data_env$data@assayData$exprs[strGene,]
		vecGenePartialNorm <- dm@data_env$relevanceMap$matNormPartials[strGene,]
		# Select highest vectors in neighbourhoods
		vecboolTopNorm <- apply(cbind(seq(1, dim(dm@data_env$relevanceMap$nn.index)[1]), dm@data_env$relevanceMap$nn.index), 1, function(cell) which.max(vecGenePartialNorm[cell]) == 1)
		...
		vecPartialDC1Gene <- dm@data_env$relevanceMap$arr3DPartials[strGene,vecboolTopNorm,1]
		vecPartialDC2Gene <- dm@data_env$relevanceMap$arr3DPartials[strGene,vecboolTopNorm,2]
...
#' ids: IDs used.
#' @export
david_plot_gene_relevance_map <- function(
	matNormPartials = NULL,
	coord = NULL,
	dm = NULL,
	smoothingiter = 2,
	ids = NULL,
	...
	if (!is.null(dm)) {
		stopifnot(!is.null(dm@data_env$relevanceMap))
		matNormPartials <- dm@data_env$relevanceMap$matNormPartials
		coord <- dm@eigenvectors[,dimnames(dm@data_env$relevanceMap$arr3DPartials)[[3]]]
	}
	...
	if (smoothingiter > 0) {
		...
		for (i in seq(1,smoothingiter)) {
			vecMaxGeneSmoothed <- apply(dm@data_env$relevanceMap$nn.index, 1, function(cell) {
	...
	gplotRelevanceMap$ids <- vecGeneIDsMap
	gplotRelevanceMap
}
